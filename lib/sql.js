...
const SQL={
  ...
  project:{
    ...
    update: 'UPDATE project SET due_date=?, state=?, completion_date=?, description=? WHERE id=?'
  },
  task:{
    ...
    update: 'UPDATE task SET state=? WHERE id=?'
  },
  work_note:{
    search:'SELECT * FROM activity WHERE tsk_id=?',
    insert: 'INSERT INTO activity SET tsk_id=?, note=?'
  }
}
